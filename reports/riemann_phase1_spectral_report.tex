...
\begin{itemize}
  ...
  \item Calcular $\Sigma^2(L)$ para $L \in [5, 50]$ con el estimador
        robusto para medir el exponente $\alpha$ con intervalos
        de confianza (bootstrap).
  \item Correlacionar los residuos del fit aritmético con las
        potencias $p^k$ (términos de orden superior de la fórmula
        explícita) para cuantificar $S(P)$ como función de $P_{\max}$.
  \item Test de persistencia en rangos disjuntos
        ($T\in[1000,1300]$ y $T\in[50000,50300]$) para determinar
        si $r(\log p, A_p) > 0.9$ es universal o de rango.
\end{itemize}
